...
def execute_superblock(prm):
    G = prm.G
    I = prm.I
    SB = prm.SB
    distances = prm.distances
    G_sb = prm.G_sb
    area_c = prm.area_c
    maxArea = prm.maxArea
    C = prm.C
    I_c = prm.I_c
    maxDist_c = prm.maxDist_c
    M = prm.M
    capacity_c = prm.capacity_c
    beta = prm.beta
    SB_subset_gc = prm.SB_subset_gc
    demand_c = prm.demand_c
    ...
    for c in range(len(C)):
        ...
        for i in I_c[c]:
            if i <= len(I_c[c]) - 1:
                m.addConstr(
                    sum(placementKey[sb, i] for sb in range(SB)) >= sum(placementKey[sb, i + 1] for sb in range(SB)))
